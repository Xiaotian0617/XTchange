package com.okcoin.okex.open.api.client;

import com.okcoin.okex.open.api.config.APIConfiguration;
import com.okcoin.okex.open.api.constant.APIConstants;
import com.okcoin.okex.open.api.enums.ContentTypeEnum;
import com.okcoin.okex.open.api.enums.HttpHeadersEnum;
import com.okcoin.okex.open.api.exception.APIException;
import com.okcoin.okex.open.api.utils.DateUtils;
import com.okcoin.okex.open.api.utils.HmacSHA256Base64Utils;
import okhttp3.*;
import okio.Buffer;
...
public class APIHttpClient {
    ...
    private Headers headers(final Request request, final String timestamp) {
        ...
        if (StringUtils.isNotEmpty(this.credentials.getSecretKey())) {
            ...
            String passphrase = this.credentials.getPassphrase();
            if (StringUtils.isNotEmpty(passphrase)){
                builder.add(HttpHeadersEnum.OK_ACCESS_PASSPHRASE.header(), passphrase);
            }
        }
    ...
    private void printRequest(final Request request, final String timestamp) {
        ...
        APIHttpClient.LOG.debug(requestInfo.toString());
    }
}
